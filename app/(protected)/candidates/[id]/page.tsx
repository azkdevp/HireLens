...
export default async function CandidateDetail({ params }: { params:Promise<{id:string}> }) {
  ...
  return <div className="stack">
    <section className="card"><div style={{ display:"flex", alignItems:"start", gap:10 }}><div style={{ marginRight:"auto" }}><span className={`badge ${c.status === "CLOSED" ? "closed" : ""}`}>{c.outcome ?? c.status}</span><h1>{c.full_name}</h1><p className="muted">{c.email} · {c.phone}</p></div>{canReview&&<div className="actions"><Link className="btn" href={`/candidates/${id}/decision-room`}>Open Decision Room</Link>{hr && <Link className="btn secondary" href={`/candidates/${id}/edit`}>Edit</Link>}</div>}</div><div className="grid2"><div><h3>Vacancy</h3><p>{c.vacancy?.title}</p></div><div><h3>Current stage</h3><p>{c.current_stage?.name}</p></div><div><h3>Source</h3><p>{c.source}</p></div><div><h3>Notes</h3><p style={{ whiteSpace:"pre-wrap" }}>{c.notes || "No notes"}</p></div></div></section>
